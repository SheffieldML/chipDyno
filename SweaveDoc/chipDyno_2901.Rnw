...
These differentially expressed data and its uncertainty doesn't contain any genes name or prolem ID number. The gene Names and its corresponding  problem IDs can be extracted from other two source (?) dictionary.txt and probeIDTu.txt.
...
Connectivity2.txt contains the evidance of connectiviety between a gene and a transcription factor. Load the connectiviety file.
<<echo=TRUE>>=
file_dataChip <- 
...
The connectiviety file depicts the relation between 6229 geses and 204 transcription factors. From the annotation file we can find the gene names present in the connectivity information. Load the annotation file. % For annotation we used ~\cite{celegans:db}.
<<echo=TRUE>>=
file_annotation <- 
...
In our experimental dataset there are some genes for which the measurement of uncertainty is zero(i.e. vars[3,]). We would like to exclude those data.
<<echo=TRUE>>=
...
The connectivity matrix between genes and the transcription factors is a binary matrix. If there is an evidance that a gene is transcribed by a transcription factor or, a transcription factor is transcribing some other genes then the relation will be 1, the value will be 0 otherwise. We binarized the matrix by giving a value based on the suggestion of ~\cite{lee:01} when the associated p-value is less than 0.0001.
<<echo=TRUE>>=
...
Optimize all the parameters using scaled conjugate gradient algorithm. This process is time consuming. The following line of command may take several hours to run. Here we can reduce the number of iteration to speed up the process and later we will load some optimized data.
...
heatmap(Sigma[1:50,1:50], Rowv=NA, Colv=NA, col = heat.colors(256),  margins=c(2,2))
@
...
%---------------------------------------------------------
\section{Computation of posterior expectations}
%---------------------------------------------------------
\Rfunction{chipDynoStatPostEstNoise} computes posterior expectations
...
<<>>=
#load("/home/muhammad/Dropbox/Git/mlprojects/chipDyno/SweaveDoc/OptimizedResults_chipDyno.RData")
#rm(list=ls())
...
\begin{figure}
<<test3, fig=TRUE>>=
image(expectations.tfErrorDiffs[,,ind], col = heat.colors(12))
@
\caption{Coariance Matrix}
\label{Cov_image}
\end{figure}

%---------------------------------------------------------
\section{Posterior expectations TFA}
%---------------------------------------------------------
\Rfunction{chipDynoExpectationsFastNoise} computes posterior expectations of transcription factor activity. The format of the function and arguments are-
...
\section{Max Diff}
%---------------------------------------------------------
\Rfunction{chipDynoMaxDiff} computes the most significant changes in TFAs.
...
%---------------------------------------------------------
\section{Significantly varying TFs}
%---------------------------------------------------------
Using \Rfunction{chipDynoActTransFactNoise} can find out identifies significantly varying TFs with uncertainty of expression level
...
<<test5, fig=TRUE>>=
plot(expectations[[1]][2,1:36], type='l', col='red')
@

\caption{TFA Activity}
\label{TFA_activiity}
\end{figure}
...
%<<test3, fig=TRUE, height=4, width=6>>=
<<test6, fig=TRUE>>=
M <- matrix(c(rep(1:4)), byrow=TRUE, nrow=2)
layout(M)
for (i in 1:6){
  plot(expectations[[1]][i,1:36], type='l', col='red')
}
@

\caption{TFA Activitys}
...
plot(x,y, type = 'l', col='green4', ylim=c(0,10.0), las=1, xlab="t", ylab="TFA", main=bquote("Gene : " ~ .(actGeneName)));
...
M <- matrix(c(rep(1:2)), byrow=TRUE, nrow=2) 
layout(M) 
...
@
\caption{TFA Activitys with error bar}
\label{TFA_activiitys_Error_Bar}
\end{figure}

%---------------------------------------------------------
\section{Given Gene: lists activators in decreasing order}
%---------------------------------------------------------
For a given gene we can find out the list of activators using the function \Rfunction{chipDynoGeneActNoise}. It will list all the activarors in decreasing order with transcription factor activity and activity error.
...
<<>>=
rm(list=ls())
load("/home/muhammad/Dropbox/Git/mlprojects/chipDyno/SweaveDoc/ResultsTu_New_10000Ita_Lovelace.RData")
source("/home/muhammad/Dropbox/Git/mlprojects/chipDyno/R/chipDynoLoadModules.R")
geneName="YHR143W"; 
geneName="AGA1"; transNames=TransNames
