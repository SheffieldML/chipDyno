...
#ker_Mat32 = GPy.kern.Matern32(input_dim=1, variance = 2.2, lengthscale=2.5)
#ker_Mat52 = GPy.kern.Matern52(input_dim=1, variance = 2.2, lengthscale=2.5)
...
plt.imshow(K)
plt.colorbar()
plt.xlabel('Brownian Kernel')

plt.subplot(2, 3, 3)
K = ker_RBF.K(X,X)
plt.imshow(K)
plt.colorbar()
plt.xlabel('EQ Kernel')
