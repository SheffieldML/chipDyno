####
#rSCGoptimize = function(f, x, options, gradf, data, X, nEffectGenes, R, C) {
##
...
#f = rChipDynoLikeStatNoise
x = params
#options = options
#gradf = rChipDynoLikeStatNoiseGrad
...
sigma0 = 1.0e-4;
fold = rChipDynoLikeStatNoise(x,data,precs, X,nEffectGenes,R,C);	# Initial function value.
fnow = fold;
...
#TODO 
gradnew = rChipDynoLikeStatNoiseGrad(x, data, precs, X, nEffectGenes, R, C);	# Initial gradient.
#gradnew=params
...
#    end
	...
	#TODO
	gplus = rChipDynoLikeStatNoiseGrad(xplus, data, precs, X, nEffectGenes, R, C)
	#gplus = params^2
...
#  alpha = - mu/delta;
	...
	fnew = rChipDynoLikeStatNoise(xnew, data, precs, X, nEffectGenes, R, C)
...
#      return;
			...
			gradold = gradnew;
			## TODO
			gradnew = rChipDynoLikeStatNoiseGrad(x, data, precs, X, nEffectGenes, R, C)
			#gradnew=params^3			
			options[11] = options[11] + 1
...
## TODO
#return(x, options, flog, pointlog, scalelog)
#}
