...
sigma0 = 1.0e-4;
fold = rChipDynoLikeStat(x,data, X,nEffectGenes,R,C);	# Initial function value.
fnow = fold;
...
#TODO 
gradnew = rChipDynoLikeStatGrad(x, data, X, nEffectGenes, R, C);	# Initial gradient.
#gradnew=params
...
#    end
	...
	#TODO
	gplus = rChipDynoLikeStatGrad(xplus, data, X, nEffectGenes, R, C)
	#gplus = params^2
...
#  alpha = - mu/delta;
	...
	fnew = rChipDynoLikeStat(xnew, data, X, nEffectGenes, R, C)
...
#      return;
			...
			gradold = gradnew;
			## TODO
			gradnew = rChipDynoLikeStatGrad(x, data, X, nEffectGenes, R, C)
			#gradnew=params^3			
			options[11] = options[11] + 1
